...
categories:
  ...
  - torque
---
![](/assets/images/kelvingrove-spitfire-figher-832x468.png)

Torque job scripts are developed in a Unix shell or a scripting language with a shell interpreter. Common examples of both approaches are Bash and Python respectively. This post will explain how to submit them to the Torque batch server
...
| -M `email_address`| -M user@email | Send email to address  when certain events occur. By default an email is sent only if the job is killed by the batch system. |
